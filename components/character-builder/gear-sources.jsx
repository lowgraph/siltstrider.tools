...
import {buildGearGroups,gearRowLabel} from '../../lib/gear-rows.mjs';

function SourceRow({row,pick,alternative=false}){
  return <tr className={alternative?'gear-alt':undefined}>
    ...
    <td><span className="gear-name">{pick?.name || 'None found'}</span>
      {pick&&<span className="gear-note">
        Base {row.category==='weapon'?'damage':row.category==='clothing'?'enchant capacity':'armor rating'}: {pick.strength}.
      ...
      <table><thead><tr><th>Slot</th><th>Item</th><th>Where</th></tr></thead><tbody>
        {groups.map(group=>{
          return group.rows.length?<Fragment key={group.label}><tr><th colSpan="3">{group.label}</th></tr>
            {group.rows.map(row=><Fragment key={row.key}><SourceRow row={row} pick={row.primary}/>{row.alternative&&<SourceRow row={row} pick={row.alternative} alternative/>}</Fragment>)}
          </Fragment>:null;
        })}
      </tbody></table>
