...
export const EquipmentLedger = memo(function EquipmentLedger({
  ...
  return (
    <div className="equipment-ledger space-y-4">
      ...
      <div
        className="p-3 bg-[#110e0a] border border-[#2b2216] mw-groove-panel flex flex-col sm:flex-row items-center justify-between gap-3 text-xs"
        style={{
          boxShadow: "inset 0 0 10px 2px rgba(0, 0, 0, 0.8)",
        }}
      >
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 border border-[#44331e] bg-[#1a140d] flex items-center justify-center font-serif font-bold text-sm text-[#d4b06a]">
            {(character.name || character.className || "C")[0].toUpperCase()}
          </div>
          <div>
            <div className="font-serif font-bold text-sm text-[#f3e6c8] tracking-wide">
              {character.name || character.className || "Vvardenfell Adventurer"}
            </div>
            <div className="text-[11px] font-serif text-[#8c7853]">
        ...
        <div className="flex items-center gap-2 font-mono">
          <div className="px-2.5 py-1 bg-[#18130c] border border-[#2e2316] flex items-center gap-1.5">
            <span className="text-[10px] uppercase font-serif text-[#8c7853]">Total AR:</span>
            <span className="font-bold text-[#d4b06a] text-sm">{totalAR}</span>
          </div>
          <div className="px-2.5 py-1 bg-[#18130c] border border-[#2e2316] flex items-center gap-1.5">
            <span className="text-[10px] uppercase font-serif text-[#8c7853]">Weight:</span>
            <span className="font-bold text-[#d4b06a] text-sm">
              {enc.totalWeight} <span className="text-[10px] text-[#8c7853]">/ {enc.maxWeight} lbs</span>
            </span>
          </div>
        </div>
      </div>
