...
export function parseOmwSave(input) {
  ...
  for (const rec of recs) {
    ...
    let isPlayer = false, race = null, cls = null, flags = null;
    for (const { tag, size, data } of subrecords(r, rec.body, rec.size)) {
      ...
      else if (tag === "FLAG" && size >= 4) flags = r.u32(data);
    }
    if (!isPlayer) continue;
    if (race) out.identity.race = race;
    if (cls) playerClassId = cls;
    /* NPC::mFlags bit 0 is Female (components/esm3/loadnpc.hpp). A save without
     * the flag word leaves gender null rather than defaulting to male. */
    if (flags !== null) out.identity.gender = (flags & 0x01) ? "Female" : "Male";
  }
  ...
  if (classId) {
    for (const rec of recs) {
      ...
      /* Specialization is 0 Combat, 1 Magic, 2 Stealth (ESM::Class::Specialization). */
      out.identity.class.specialization = ["Combat", "Magic", "Stealth"][r.i32(cldt + 8)] ?? null;
      out.identity.class.favoredAttributes = [r.i32(cldt), r.i32(cldt + 4)]
        .map((index) => ATTRIBUTE_IDS[index] ?? null).filter(Boolean);
