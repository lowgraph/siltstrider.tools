...
export { ATTRS, ATTR_ABBR, SPECIALIZATIONS, SKILL_SPEC, SKILL_GOV, applyBitterCup };
 ...
 */
export function normalizeCharacterState(character, catalogs, options = {}) {
  if (!character) {
  ...
  let sheet = null;
  let build = character;
  const wantsBitterCup = Boolean(options.bitterCup || character.bitterCup);

  // If already a computed sheet (has attrs and skills objects)
  if (character.attrs && character.skills) {
    sheet = character;
  } else if (catalogs) {
    sheet = computeSheet({ ...character, bitterCup: wantsBitterCup }, catalogs);
  }
  ...
  // If Bitter Cup requested but sheet was either not computed or didn't have Bitter Cup applied
  let bitterCupData = sheet?.bitterCup || null;
  if (wantsBitterCup && !bitterCupData) {
    bitterCupData = applyBitterCup(attributes);
    for (const attr of ATTRS) {
      attributes[attr] = bitterCupData.modifiedAttributes[attr];
    }
  }
  ...
  if (wantsBitterCup && !sheet?.bitterCup && (character.health === undefined || options.bitterCup)) {
    health = Math.floor(((attributes.Strength || 40) + (attributes.Endurance || 40)) / 2);
  }
...
export function simulateProgression(character, options = {}) {
  const initialSheet = normalizeCharacterState(character, options.catalogs, options);
  ...
  const levelCap = isStatsOnly ? Math.max(100, initialSheet.level) : initialSheet.levelCap;
  ...
  while (state.level < targetLevel) {
    let step = optimizeLevelStep(state, {
      priority,
    ...
    const override = options.manualSteps?.[state.level];
    if (override) {
      const validation = validateLevelStep(state, override, {mode: options.mode, statsOnly: isStatsOnly});
      if (!validation.valid) throw new Error(validation.errors.join(' '));
      step = {...step, ...override, manual: true, attributeBonuses:override.attributeBonuses.map(pick => ({...pick,
        startValue:state.attributes[pick.attribute], endValue:Math.min(100,state.attributes[pick.attribute]+pick.bonus), multiplier:pick.bonus
      }))};
    }
    ...
    step = {...step, healthGain:state.lastStep.healthGain, newHealth:state.health};
 ...
 * @param {Object} [options] Options including bitterCup
 * @returns {Object} { levels, optimalHealth, delayedHealth, difference, maxDifference }
 */
export function calculateHealthGrowthCurve(character, targetLevel, catalogs, options = {}) {
  const base = normalizeCharacterState(character, catalogs, options);
  ...
  const delayedEndurance = [base.attributes.Endurance];
  ...
  return {
    ...
    optimalEndurance: optimalEndurance.slice(0, len),
    delayedEndurance: delayedEndurance.slice(0, len),
    // First level at which the rushed path has Endurance 100 (every later level-up gives the full Health gain).
    enduranceMaxLevel: levels.slice(0, len).find((lvl, i) => optimalEndurance[i] >= 100) ?? null
  };
}
