  ...
  if (Array.isArray(skills)) {
    ...
    if (Array.isArray(majorMinorNames)) {
      ...
      if (names.length === 10) {
        ...
        const numericValues = Object.values(skills).filter(
          (v) => typeof v === "number" || (typeof v === "object" && v !== null && "v" in v)
        );
        skillValues = numericValues.slice(0, 10);
      }
    }
  }
...
export function validateLevelStep(currentState, stepData, options = {}) {
  ...
  const isStatsOnly =
    ...
    stepData.mode === PROGRESSION_MODES.STATS_ONLY ||
    Boolean(stepData.statsOnly);

  const { attributes, skills, maj = [], min = [], misc = [] } = currentState;
...
export function optimizeLevelStep(currentState, options = {}) {
  ...
  if (uncappedAttrs.length === 0) {
    ...
    return {
      ...
      newHealth: currentState.health + calculateHealthGain(attributes.Endurance),
  ...
  if (isStatsOnly) {
    ...
    return {
      ...
      healthGain,
      newEndurance,
      newHealth: currentState.health + healthGain,
...
export function simulateProgression(character, options = {}) {
  ...
  const levelCap = initialSheet.levelCap;
  const targetLevel = options.targetLevel ? Math.min(options.targetLevel, levelCap) : levelCap;
