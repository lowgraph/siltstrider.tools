...
export const FEATURE_CATALOGS = Object.freeze({
  ...
  equipment: Object.freeze(['Weapons','Armor','Clothing','Enchantments']),
  alchemy: Object.freeze(['Ingredients','Apparatus','MagicEffects','GameSettings']),
  enchanting: Object.freeze(['MagicEffects','GameSettings','Enchantments']),
  spellmaking: Object.freeze(['MagicEffects','GameSettings']),
...
export function createBundleLoader({baseUrl='/game-data/',fetcher=globalThis.fetch?.bind(globalThis),cacheStorage=globalThis.caches,crypto=globalThis.crypto}={}) {
  ...
  async function loadCatalog(profileId,name){
    ...
    return once(profileId+':'+name,async()=>{
      ...
      const data=await payload(m,p,name,f);
  ...
  async function loadFeature(profile,feature){
    ...
    return freeze({bundleId:m.bundleId,snapshotId:m.snapshotId,profile,catalogs:Object.fromEntries(names.map((name,i)=>[name,values[i]]))});
  }
  return Object.freeze({manifest,loadCatalog,loadFeature});
}
