...
/**
 * A run from a shared link, kept to the fields and sizes a run has; null when it holds
 * no run. Link contents come from whoever made the link, so nothing else is let in.
 */
export function sanitizeRun(input) {
  if (!input || typeof input !== "object" || Array.isArray(input)) return null;
  const text = (value, max = 120) => (typeof value === "string" ? value.slice(0, max) : "");
  const texts = (value, max = 5, length = 200) =>
    Array.isArray(value) ? value.filter((v) => typeof v === "string").slice(0, max).map((v) => v.slice(0, length)) : [];
  const run = createEmptyRun(text(input.seed, 40));
  run.race = text(input.race);
  run.gender = input.gender === "Female" || input.gender === "Male" ? input.gender : "";
  run.cls = text(input.cls);
  run.sign = text(input.sign);
  run.spec = ["Combat", "Magic", "Stealth"].includes(input.spec) ? input.spec : run.spec;
  if (ATTRS.includes(input.fav1)) run.fav1 = input.fav1;
  if (ATTRS.includes(input.fav2)) run.fav2 = input.fav2;
  run.maj = texts(input.maj).filter((s) => ALL_SKILLS.includes(s));
  run.min = texts(input.min).filter((s) => ALL_SKILLS.includes(s));
  run.major = text(input.major, 300);
  run.minors = (Array.isArray(input.minors) ? input.minors : []).slice(0, 5).map((o) =>
    typeof o === "string" ? { kind: "", text: o.slice(0, 300) } : { kind: text(o?.kind, 20), text: text(o?.text, 300) }
  ).filter((o) => o.text);
  run.rests = texts(input.rests, 5, 300);
  const vitals = input.vitals;
  run.vitals = vitals && ["health", "magicka", "fatigue"].every((k) => Number.isFinite(vitals[k]))
    ? { health: vitals.health, magicka: vitals.magicka, fatigue: vitals.fatigue }
    : null;
  run.seedExact = input.seedExact !== false;
  return run.race || run.cls || run.sign || run.major || run.rests.length || run.minors.length ? run : null;
}
