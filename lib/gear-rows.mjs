...
export function buildGearGroups(catalogs,build,toggles,ranking,{beast=false}={}){
  ...
  const rows=catalogs.GearRows.filter(r=>Object.keys(DEFAULT_GEAR_TOGGLES).every(k=>r.toggles[k]===toggles[k]))
  .filter(r=>!ranking.weaponSetup || (r.category==='weapon' ? r.hands===(ranking.weaponSetup==='two-handed'?2:1) : !(ranking.weaponSetup==='two-handed' && r.category==='shield'))).map(row=>{
