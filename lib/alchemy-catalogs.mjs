...
export function adaptAlchemy(data,legacyEffects=[]){
  ...
  const effects=Object.fromEntries(c.MagicEffects.map(r=>{
    const engine=c.EffectRules?.find(e=>String(e.key)===String(r.key));
    const rule=engine ? {mag:typeof engine.noMagnitude!=='boolean'?undefined:Number(!engine.noMagnitude),dur:typeof engine.noDuration!=='boolean'?undefined:Number(!engine.noDuration),harmful:engine.harmful} : legacyEffects.find(e=>e.n===r.name);
    return [r.key,{...rule,n:r.name,b:r.baseCost,supported:!!rule&&[0,1].includes(rule.mag)&&[0,1].includes(rule.dur)&&(!engine||typeof rule.harmful==='boolean')&&Number.isFinite(r.baseCost)&&r.baseCost>0,source:r}];
  }));
  ...
  const ingredients=c.Ingredients.map(r=>{
    ...
    for(const e of r.effects){
      ...
      ordered[e.slot]={...effect,id:String(e.effectId),arg,bad:effect.harmful===true};
    }
